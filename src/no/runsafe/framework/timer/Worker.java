...
public abstract class Worker<TokenType, StateType> implements Runnable
{
	...
	public void run()
	{
		...
		StateType value = state.getOrDefault(key, null);
		state.remove(key);
		process(key, value);
	}
	...
	private final ConcurrentHashMap<TokenType, StateType> state = new ConcurrentHashMap<>();
	private final Stack<TokenType> queue = new Stack<>();
}